...
export const bankApprove: RequestHandler = async (req, res) => {
  ...
  try {
    const projects = await getCollection("projects");
    const proj = await projects.findOne({ id: (doc as any).projectId });
    const { sendOtpEmail } = await import("../utils/mailer");
    if (proj?.email || proj?.ownerEmail) {
      await sendOtpEmail((proj as any).email || (proj as any).ownerEmail, `Payout released for ${proj?.id}`);
    }
  } catch {}
  res.json({ ok: true });
};
