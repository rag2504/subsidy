...
function Programs() {
  ...
  return (
    <section className="rounded-xl border bg-card p-6 shadow-sm">
      ...
      <form className="mt-3 flex gap-2" onSubmit={async (e) => { e.preventDefault(); await fetch("/api/gov/programs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) }); setName(""); load(); }}>
...
function PendingProjects() {
  ...
  const load = () => fetch("/api/gov/projects?status=pending").then((r) => r.json()).then(setList);
  useEffect(() => { load(); }, []);
  return (
    <section className="rounded-xl border bg-card p-6 shadow-sm">
      ...
      <ul className="mt-3 grid gap-2">
        {list.map((p) => (
          <li key={p.id} className="flex items-center justify-between rounded-md border p-3 text-sm">
            ...
            <Button onClick={async () => { await fetch(`/api/gov/projects/${p.id}/approve`, { method: "POST" }); load(); }}>Approve</Button>
          </li>
        ))}
      </ul>
    </section>
...
function Release() {
  ...
  return (
    <section className="rounded-xl border bg-card p-6 shadow-sm">
      ...
      <form className="mt-3 grid gap-2 md:grid-cols-5" onSubmit={async (e) => { e.preventDefault(); setMsg(null); const r = await fetch("/api/gov/release", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, milestoneKey, amount: Number(amount), rail }) }); setMsg(r.ok ? "Queued for bank" : "Error"); }}>
  ...
  return (
    <section className="rounded-xl border bg-card p-6 shadow-sm">
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-3">
        <input className="rounded-md border px-3 py-2 text-sm" placeholder="Project ID" value={projectId} onChange={(e) => setProjectId(e.target.value)} />
        <Button onClick={async () => { await fetch("/api/gov/revoke", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, reason: "policy" }) }); }}>Revoke</Button>
        <div />
        ...
        <Button onClick={async () => { await fetch("/api/gov/clawback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, amount: Number(amount), reason: "violation" }) }); }}>Clawback</Button>
      </div>
    </section>
  );
}
