    ...
    <AuthGate requiredRole="auditor">
      <div className="space-y-6">
        ...
        <section className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          <form className="mt-3 grid gap-2 md:grid-cols-6" onSubmit={async (e) => { e.preventDefault(); setMsg(null); const r = await fetch("/api/auditor/attest", withAuthHeaders({ method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, milestoneKey, value: Number(value), unit, dataHash, signer }) })); setMsg(r.ok ? "Submitted" : "Error or already exists"); }}>
