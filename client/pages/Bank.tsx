...
function Approve({ id, onDone }: { id: string; onDone: () => void }) {
  ...
  return (
    <form className="flex items-center gap-2" onSubmit={async (e) => { e.preventDefault(); setBusy(true); await fetch("/api/bank/approve", withAuthHeaders({ method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, bankRef: ref }) })); setBusy(false); onDone(); }}>
