    ...
    <AuthGate requiredRole="producer">
      <div className="space-y-6">
        ...
        <section className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          <form className="mt-3 grid gap-2 md:grid-cols-4" onSubmit={async (e) => { e.preventDefault(); const r = await fetch("/api/producer/projects", withAuthHeaders({ method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ programId, name, email }) })); const data = await r.json(); setApplied(data); }}>
